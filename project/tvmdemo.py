...
def compile():
    model, device = image_depth.get_tvm_model()
    SO_B, SO_C, SO_H, SO_W = 1, 3, model.MAX_H, model.MAX_W

    todos.data.mkdir("output")
    if not os.path.exists("output/image_depth.so"):
        input = torch.randn(SO_B, SO_C, SO_H, SO_W)
        todos.tvmod.compile(model, device, input, "output/image_depth.so")
...
def predict(input_files, output_dir):
    ...
    # load model
    tvm_model = todos.tvmod.load("output/image_depth.so", str(device))
